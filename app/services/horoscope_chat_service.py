...
INTENT_KEYWORDS: Dict[str, tuple[str, ...]] = {
    "career": ("job", "career", "work", "promotion"),
    "marriage": ("marriage", "wedding", "love"),
    "finance": ("money", "finance", "financial", "income", "salary", "earnings"),
}
...
class HoroscopeChatService:
    ...
    def analyze_query(self, query: str, user_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        detected_intent = self.detect_intent(normalized_query)
        resolved_intent = self._resolve_intent_with_memory(
            normalized_query,
            detected_intent,
            conversation_memory,
        )
    ...
    def _resolve_intent_with_memory(
        self,
        query: str,
        detected_intent: str,
        ...
        last_intent = str(recent_queries[-1].get("intent", "general")).strip().lower()
    ...
    def _build_memory_note(
        ...
        previous_intent = str(recent_queries[-1].get("intent", "general")).strip().lower()
        if previous_intent == intent:
            return f"This follows your earlier {intent} question."
        return f"This follows your earlier {previous_intent} question, now focusing on {intent}."
